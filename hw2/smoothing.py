...
def text_classification(key):
    ...
    if class_label == "truthful positive":
        word_count_update(review_str, tp_dic, tp_word_count,tp_review_count)
    else:  
        if class_label == "deceptive positive":
            word_count_update(review_str, dp_dic, dp_word_count,dp_review_count)
        else: 
            if class_label == "truthful negative":
                word_count_update(review_str, tn_dic, tn_word_count,tn_review_count)
            else:    # grade must D or F
                if class_label == "deceptive negative":
                    word_count_update(review_str, dn_dic, dn_word_count,dn_review_count)
...
#smoothing
for key in dic:
 if tp_dic.has_key(key)==False:
  tp_dic[key]=1.0
  tp_word_count[0]=tp_word_count[0]+1.0
 if dp_dic.has_key(key)==False:
  dp_dic[key]=1.0
  dp_word_count[0]=dp_word_count[0]+1.0
 if tn_dic.has_key(key)==False:
  tn_dic[key]=1.0
  tn_word_count[0]=tn_word_count[0]+1.0
 if dn_dic.has_key(key)==False:
  dn_dic[key]=1.0
  dn_word_count[0]=dn_word_count[0]+1.0
...
prior_pro['tp'] = tp_review_count[0]/total_review_count[0]
prior_pro['dp'] = dp_review_count[0]/total_review_count[0]
prior_pro['dn'] = dn_review_count[0]/total_review_count[0]
prior_pro['tn'] = tn_review_count[0]/total_review_count[0]
